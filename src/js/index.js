...
function render() {
	document
		.querySelector(this.options.parent)
		.insertAdjacentHTML(
			"beforeend",
			'<div class="jProgress"><div class="bar"></div><div class="barBg"></div></div>'
		);
}
...
function styleBar($jProgress, $bar, $barBg, context) {
	$jProgress.setAttribute(
		"style",
		"height: " +
			context.options.progressHeight +
			"px; height: " +
			context.options.progressHeight / 10 +
			"rem; " +
			"z-index: " +
			context.options.zIndex +
			";" +
	...
	$bar.setAttribute(
		"style",
		"background-color: " +
			context.options.brandColor +
			";" +
			"width: 1px; height: 100%; transition: all 500ms ease-in-out;"
	);
